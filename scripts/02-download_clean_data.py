...
standardized_df = renamed_df.with_columns([
    #for sexual orientation
    pl.when(pl.col("sexual orientation").str.contains(r"(?i)\b2s*lgbtq\+\b"))
      .then(pl.lit("2SLGBTQ+"))
      .when(pl.col("sexual orientation").str.contains(r"(?i)\blesbian\b"))
      .then(pl.lit("Lesbian"))
      .otherwise(pl.col("sexual orientation"))
      .alias("sexual orientation"),

    #for gender
    pl.when(pl.col("gender").str.contains(r"(?i)transgender woman"))
      .then(pl.lit("Transgender Woman"))
      .otherwise(pl.col("gender"))
      .alias("gender")
    
])

clean = standardized_df.filter(
    (pl.col("location_type") != "NA") &
    (pl.col("neighbourhood") != "NSA") &
    (pl.col("primary_offence") != "NA") &
    (pl.col("primary_offence") != "This should be removed")
)
